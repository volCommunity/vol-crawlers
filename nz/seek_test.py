...
class SeekSpiderTest(unittest.TestCase):
    ...
    def test_parse(self):
        ...
        results = self.spider.parse_org_page(
            fake_response_from_file(
                'nz_seek/volunteering-organisations_903_west-auckland-academic-mentoring.html'))
    ...
    def post_job(self, item):
        ...
        j = r.json()
        if j['count'] != 0:
            r = requests.delete('http://localhost:8000/api/jobs/{0}'.format(j['results'][0]['id']),
                                headers=headers)
            r.raise_for_status()
        ...
        for l in item['labels']:
            r = requests.get('http://localhost:8000/api/labels',
                             params={'name': l},
                             headers=headers)
            j = r.json()
            if j['count'] > 0:
                ...
                r = requests.post('http://localhost:8000/api/labels',
                                  json={'name': l},
                                  headers=headers)
                j = r.json()
                tmp.append(j['id'])
        ...
        r = requests.get('http://localhost:8000/api/organisations',
        ...
        if j['count'] > 0:
            ...
            org_object = self.spider.parse_org_page(
                fake_response_from_file(
                    'nz_seek/volunteering-organisations_903_west-auckland-academic-mentoring.html'))
            ...
            r = requests.post('http://localhost:8000/api/organisations',
                              json=org,
                              headers=headers)
            j = r.json()
        ...
        for i in item['sites']:
            ...
            if j['count'] > 0:
                ...
                r = requests.post('http://localhost:8000/api/sites',
                                  json=site,
